...
export async function register(username, challenge, options) {
  ...
  if (options.debug) alert(JSON.stringify(creationOptions));
  alert("credentials before");
  const credential = await navigator.credentials.create({
    publicKey: creationOptions,
  }); //PublicKeyCredential
  alert("credentials after");
  if (options.debug) alert(JSON.stringify(credential));
...
export async function authenticate(credentialIds, challenge, options) {
  ...
  if (options.debug) alert(JSON.stringify(authOptions));
  let auth = await navigator.credentials.get({
    publicKey: authOptions,
    mediation: options.mediation,
  });
  if (options.debug) alert(JSON.stringify());
